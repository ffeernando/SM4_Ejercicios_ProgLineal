...
### Creación del Solver
prob = pulp.LpProblem("Maximizar Rentabilidad", pulp.LpMaximize)

### Creación de variables x_g,k
x = pulp.LpVariable.dicts("x", ((g, k) for g in grupos for k in range(1, k_max + 1)), lowBound=0, cat='Integer')

### Función objetivo
prob += pulp.lpSum(r_g[g, k] * x[g, k] for g in grupos for k in range(1, k_max + 1)), "Rentabilidad Total"

### Restricción del total de visitas
prob += pulp.lpSum(k * x[g, k] for g in grupos for k in range(1, k_max + 1)) == total_visitas, "Total Visitas"

### Restriccion de visitas minimas
for g in grupos:
    prob += pulp.lpSum(x[g, k] for k in range(1, k_max + 1)) == poblaciones[g], f"Población_{g}"
...
if pulp.LpStatus[prob.status] == 'Optimal':
    ...
    for g in grupos:
        for k in range(1, k_max + 1):
            print(f"Visitas al grupo {g}, {k}-ésima visita: {x[g, k].varValue}")
else:
